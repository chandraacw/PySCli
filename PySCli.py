...
parser.add_argument(
    "-t",
    "--type",
    help="Type to be Downloaded (video/audio)",
    choices=["video", "audio"],
    required=False,
    default="video",
    dest="typen",
...
parser.add_argument(
    ...
    "--output-path",
    dest="opath",
    help="Output path for installed video/audio (Default is CWD)",
    type=pathlib.Path,
    default=os.getcwd(),
)
parser.add_argument(
    "-N",
    "--no-thumbnail",
    help="Download Thumbnail Exception",
    action="store_true",
    required=False,
    dest="is_thumb",
)
...
def download_vid(url, save, type, no_thumbnail=False):
    ...
    if type.lower() == "video":
        stream = yt.streams.filter(
            progressive=True, file_extension=fex
